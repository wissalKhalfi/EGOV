...
// If is a admin or super admin we redirect to the backoffice area
        ...
        if (in_array('ROLE_CITOYEN', $rolesTab, true)) {
            $redirection = new RedirectResponse($this->router->generate('citoyen'));
        }
// otherwise, if is a commercial user we redirect to the crm area
        elseif (in_array('ROLE_SUPER_ADMIN', $rolesTab, true)) {
            $redirection = new RedirectResponse($this->router->generate('administrateur'));
        }
         elseif (in_array('ROLE_ENTREPRISE', $rolesTab, true)) {
            $redirection = new RedirectResponse($this->router->generate('entreprises'));
        }
